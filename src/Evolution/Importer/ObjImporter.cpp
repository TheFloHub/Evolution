...
ObjImporter::ObjImporter(string objFileName)
...
void ObjImporter::readModel(ifstream & objFile)
{
  string line;
...
bool ObjImporter::addFace(string & line)
{
  ...
  int vi, ti, ni;
  for (faceTokenIndex = 1, vertexIndex = 0; faceTokenIndex < 4;
       ++faceTokenIndex, ++vertexIndex)
  {
...
bool ObjImporter::read3Floats(const string & line, Point3f & p)
{
...
bool ObjImporter::read2Floats(const string & line, Point2f & p)
{
  vector<string> strings = getTokens(line, ' ');
  if (strings.size() != 3)
    return false;
...
vector<string> ObjImporter::getTokens(const string & line, const char c)
{
...
void ObjImporter::trim(string & str)
{
...
void ObjImporter::centerScale()
{
  ...
  for (vector<Point3f>::iterator it = mVertexList.begin();
       it != mVertexList.end(); ++it)
  {
    if (it->x > rightPt)
      rightPt = it->x;
    if (it->x < leftPt)
  ...
  Point3f center((rightPt + leftPt) / 2.0f, (topPt + bottomPt) / 2.0f,
                 (nearPt + farPt) / 2.0f);
...
void ObjImporter::calculateNormals()
{
  ...
  for (vector<Point3i>::iterator it = mIndices.begin(); it != mIndices.end();
       ++it)
  {
    a = mVertexList[it->x];
    b = mVertexList[it->y];
  ...
  for (vector<vector<Point3f>>::iterator it1 = mCalculatedNormals.begin();
       it1 != mCalculatedNormals.end(); ++it1)
  {
...
bool ObjImporter::isSuccessfullyLoaded() const { return mFileOkay; }

std::vector<GLfloat> const ObjImporter::getVertices() const
{
...
std::vector<GLfloat> const ObjImporter::getTextureCoords() const
{
  std::vector<GLfloat> texCoords;
  texCoords.reserve(2 * mNewTextureList.size());
  for (auto iter = mNewTextureList.cbegin(); iter < mNewTextureList.cend();
       ++iter)
  {
...
std::vector<GLfloat> const ObjImporter::getNormals() const
{
  std::vector<GLfloat> normals;
  normals.reserve(3 * mNewNormalList.size());
  for (auto iter = mNewNormalList.cbegin(); iter < mNewNormalList.cend();
       ++iter)
  {
...
std::vector<GLuint> const ObjImporter::getIndices() const
{
